    ...
    def resolve(self, restitution=None, rebound=None, friction=None, update=True):
        ...
        if self.overlap > 0:
            ...
            if v_rel_dot < 0 or rebound != 0:
                new_mass = 1/(1/a.mass + 1/b.mass + (sa.cross(self.normal))**2/a.momi + (sb.cross(self.normal))**2/b.momi)
                Jn = -(1 + restitution) * new_mass * v_rel_dot + m * rebound

                # Find our tangent direction vector (t_hat)
                t = -self.normal.rotate(90)
                # Make sure it is opposite to the direction of relative velocity
                if relative_velocity.dot(t) > 0:
                    t = -t

                # Calculate Jt needed for static friction
                Jt = -relative_velocity.dot(t) * m

                # Check if Jt is too strong
                if Jt > friction * Jn:
                # if so, calculate the new (weaker) Jt
                    Jt = friction * Jn
                else:
                    # Otherwise, correct for creep
                    denom = (relative_velocity.dot(self.normal))
                    if denom != 0:
                        creep = (relative_velocity.dot(t)) / (relative_velocity.dot(self.normal)) * self.overlap
                        a.pos = a.pos + t * (creep * (m/a.mass))
                        b.pos = b.pos - t * (creep * (m/b.mass))
                        a.update(0)
                        b.update(0)

                impulse = Jn * self.normal + Jt * t # <--- add the tangential impulse
...
class Polygon_Wall(Contact):
    def update(self):
        polygon:Polygon = self.a
        wall:Wall = self.b
        self.overlap = -(math.inf)
        for i, point in enumerate(polygon.points):
            # Check overlap of the current point
            overlap = -(point - wall.pos) * wall.normal
    ...
    def point(self):
        polygon:Polygon = self.a
        #return circle.pos - circle.radius * self.normal
        return polygon.points[self.index]

class Polygon_Polygon(Contact):
    def update(self):
        polygonA: Polygon = self.a
        polygonB: Polygon = self.b
        self.overlap = math.inf
        self.normal = None
        self.index = -1
        self.polygon = None
        ...
        # A against B walls
        for i in range(len(polygonB.points)):
            wall_overlap = -math.inf
            n = polygonB.normals[i]
            wall_pt = polygonB.points[i]

            for j, p in enumerate(polygonA.points):
                overlap = (wall_pt - p).dot(n)
            ...
            if wall_overlap < self.overlap:
                ...
                self.polygon = polygonA

        # B against A walls
        for i in range(len(polygonA.points)):
            wall_overlap = -math.inf
            n = polygonA.normals[i]
            wall_pt = polygonA.points[i]

            for j, p in enumerate(polygonB.points):
                overlap = (wall_pt - p).dot(n)
