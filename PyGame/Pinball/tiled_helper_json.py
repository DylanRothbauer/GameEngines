...
def parse_object(o):
        ...
        if "pivot" in kwargs:
            for p in raw_objects:
                if p["id"] == kwargs["pivot"]:
                    ...
                    del kwargs["pivot"]
                    break
        shape = Polygon(pos=pos, local_points=points, name=o["name"], type=o["type"], angle=math.radians(o["rotation"]), **kwargs)
        if pivot:
            for p in shape.local_points:
                p -= pivot.rotate_rad(-shape.angle)
            shape.pos += pivot
            shape.update(0)
        return shape
    ...
    # Circle (squares are interpreted as circles)
        ...
        if o["width"] == o["height"]:  
            ...
            return Circle(pos=center, radius=o["width"]/2, name=o["name"], type=o["type"], angle=math.radians(o["rotation"]), **kwargs)
        else:
